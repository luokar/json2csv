...
export function useRelationalPreview(
  ...
  const pendingProjectionPatchRef = useRef<Partial<RelationalProjectionState> | null>(null);
  ...
  const { customJson, includeRelational, rootPath, sampleJson, sourceMode } = request;
  ...
  useEffect(() => {
    ...
    if (!workerRef.current) {
      workerRef.current = new Worker(new URL("../workers/relational-worker.ts", import.meta.url), {
        type: "module",
      });
    }

    const worker = workerRef.current;
    const handleMessage = (event: MessageEvent<ProjectionRelationalWorkerResponse>) => {
