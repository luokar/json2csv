  ...
  it('returns null for small or already narrowed roots', () => {
    expect(
      buildComplexJsonOverview(
        [createInspectedPath('name', ['string'])],
        1,
        '$',
      ),
    ).toBeNull()
  ...
  it('summarizes broad complex roots into candidate branches', () => {
    const overview = buildComplexJsonOverview(
      [
        ...
        ...Array.from({ length: 1_300 }, (_, index) =>
          createInspectedPath(
            `components.schemas.Model_${index}.properties.id`,
            ['object'],
          ),
        ),
        {
    ...
    expect(overview?.topLevelBranches.map((branch) => branch.path)).toContain(
      'paths',
    )
    expect(overview?.topLevelBranches.map((branch) => branch.path)).toContain(
      'components',
    )
    expect(overview?.candidateRoots.map((branch) => branch.path)).toContain(
      'components.schemas',
    )
    expect(overview?.candidateRoots.map((branch) => branch.path)).toContain(
      'paths',
    )
    expect(overview?.candidateRoots.map((branch) => branch.path)).not.toContain(
      'paths.route_0',
    )
    expect(overview?.candidateRoots.some((branch) => branch.hasArray)).toBe(
      true,
    )
  })
