...
export type HangAuditCategory = "intent" | "frame-gap" | "longtask" | "recovered" | "transition";
...
export function createEmptyHangAuditSnapshot(now: number = Date.now()): HangAuditSnapshot {
  return {
    activeIntent: null,
...
export function getNextHangAuditEntryId(snapshot: HangAuditSnapshot) {
  return snapshot.entries.reduce((maxId, entry) => Math.max(maxId, entry.id), 0) + 1;
}
...
export function readInitialHangAuditSnapshot(now: number = Date.now()): HangAuditSnapshot {
  if (typeof window === "undefined") {
...
function normalizeHangAuditSnapshot(value: unknown): HangAuditSnapshot | null {
  ...
  const activeTransition = normalizeHangAuditTransition(candidate.activeTransition);
...
function normalizeHangAuditTransition(value: unknown): HangAuditTransitionState | null {
  if (!value || typeof value !== "object") {
    return null;
  }
...
function normalizeHangAuditContext(value: unknown): HangAuditContext {
  ...
  return {
    columnCount: typeof candidate.columnCount === "number" ? candidate.columnCount : 0,
    customJsonChars: typeof candidate.customJsonChars === "number" ? candidate.customJsonChars : 0,
    isProjecting: candidate.isProjecting === true,
    isWorkbenchSuspended: candidate.isWorkbenchSuspended === true,
    projectionLabel:
      typeof candidate.projectionLabel === "string" ? candidate.projectionLabel : null,
    ...
    transitionLabel:
      typeof candidate.transitionLabel === "string" ? candidate.transitionLabel : null,
    transitionPhase:
      typeof candidate.transitionPhase === "string" ? candidate.transitionPhase : null,
  };
}
