...
export function resolveStreamableJsonPath(rootPath: string): StreamableJsonPath | null {
...
export function streamJsonPath(
  ...
  const resolvedPath =
    typeof rootPath === "string" ? resolveStreamableJsonPath(rootPath) : rootPath;
...
function streamArrayMatches(
  ...
  while (true) {
    if (selectedIndex === null || entryIndex === selectedIndex) {
      matchedPath =
        streamJsonPathMatches(parser, tokens, tokenIndex, onAdvance, onMatch) || matchedPath;
    } else {
      parser.skipValue();
      onAdvance();
    }
...
class JsonTextParser {
  ...
  error(message: string) {
    return new Error(`Invalid JSON input at character ${this.index + 1}: ${message}`);
  }
