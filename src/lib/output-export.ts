...
export function buildOutputExportBundle(
  ...
  const exportBaseName = sanitizeExportSegment(
    request.exportName,
    defaultExportBaseName,
  )
  const flatCsv = createTextArtifact(
    `${exportBaseName}.csv`,
    flatResult.csv,
    csvMimeType,
  )
  const relationalTables = relationalResult.tables.map((table) => {
    const tableFileName = createRelationalTableFileName(
      exportBaseName,
      table.tableName,
    )
...
function createRelationalArchive(
  ...
  const tableEntries: Record<string, Uint8Array> = {
    'README.txt': normalizeBytes(
      strToU8(
        'Each CSV is a normalized relational table derived from the current root selection.',
      ),
    ),
  }
  const archiveEntries = {
    [directory]: {
      'manifest.json': normalizeBytes(
...
function createRelationalTableFileName(
  exportBaseName: string,
  tableName: string,
) {
